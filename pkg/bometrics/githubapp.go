...
func githubAppCredentials(ctx context.Context, client client.Client) (int64, []byte, error) {
	...
	if err := client.Get(ctx, globalPaCSecretKey, &pacSecret); err != nil {
		return 0, nil, boerrors.NewBuildOpError(boerrors.EPaCSecretNotFound,
			fmt.Errorf("pipelines as Code secret not found in %s namespace", common.BuildServiceNamespaceName))
	}
	config := pacSecret.Data
	githubAppIdStr := string(config[common.PipelinesAsCodeGithubAppIdKey])
	...
	privateKey := config[common.PipelinesAsCodeGithubPrivateKey]
	if len(config[common.PipelinesAsCodeGithubPrivateKey]) == 0 {
