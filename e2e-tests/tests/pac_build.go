...
var _ = framework.BuildSuiteDescribe("Build service E2E tests", Label("build-service"), func() {
	...
	pacBuildEntries := GetEnabledProviderEntries()
	DescribeTableSubtree("test git provider", Label("github-webhook", "pac-build", "pipeline", "image-controller"), func(gitProvider git.GitProvider, gitPrefix string) {
			...
			When("the component is removed", Label("build-custom-branch"), func() {
				...
				It("purge PR is created successfully", func() {
					Eventually(func() bool {
						...
						return false
					}, 5*time.Minute, 10*time.Second).Should(BeTrue(), fmt.Sprintf("timed out when waiting for purge PR with target branch %s to be created in %s repository", componentBaseBranchName, helloWorldRepository))
				})
			})
		})
	}, pacBuildEntries)
