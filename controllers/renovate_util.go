...
func GetAllGithubInstallations(ctx context.Context, client client.Client, eventRecorder record.EventRecorder, componentList []v1alpha1.Component) (string, []installationStruct, error) {
	...
	if err := client.Get(ctx, globalPaCSecretKey, &pacSecret); err != nil {
		...
		if errors.IsNotFound(err) {
			log.Error(err, "not found Pipelines as Code secret", "namespace", globalPaCSecretKey.Namespace, err, logs.Action, logs.ActionView)
		} else {
			log.Error(err, "failed to get Pipelines as Code secret", "namespace", globalPaCSecretKey.Namespace, err, logs.Action, logs.ActionView)
		}
		return "", nil, nil
	}
