...
var GenerateRenovateConfigForNudge func(target updateTarget, buildResult *BuildResult, gitRepoAtShaAnnotation string) (RenovateConfig, error) = generateRenovateConfigForNudge
...
func (u ComponentDependenciesUpdater) GetUpdateTargetsBasicAuth(ctx context.Context, componentList []v1alpha1.Component, imageRepositoryHost, imageRepositoryUsername, imageRepositoryPassword string) []updateTarget {
	...
	for _, component := range componentList {
		...
		if err != nil {
			log.Error(err, "failed to read custom renovate config map", "ComponentName", component.Name, "ComponentNamespace", component.Namespace)
		}
...
// ReadCustomRenovateConfigMaps This method returns custom renovate options from config map
func (u ComponentDependenciesUpdater) ReadCustomRenovateConfigMap(ctx context.Context, component *v1alpha1.Component) (CustomRenovateOptions, error) {
	...
	customComponentRenovateConfigMap := readComponentCustomRenovateConfigMapAnnotation(component)

	customRenovateConfigMapName := NamespaceWideRenovateConfigName
	if customComponentRenovateConfigMap != "" {
		customRenovateConfigMapName = customComponentRenovateConfigMap
	}

	customRenovateConfigMap := &corev1.ConfigMap{}
	log.Info("trying to get custom renovate config map", "configMapName", customRenovateConfigMapName)
	if err := u.Client.Get(ctx, types.NamespacedName{Name: customRenovateConfigMapName, Namespace: component.Namespace}, customRenovateConfigMap); err != nil {
		if errors.IsNotFound(err) {
			if customRenovateConfigMapName != NamespaceWideRenovateConfigName {
...
func generateRenovateConfigForNudge(target updateTarget, buildResult *BuildResult, gitRepoAtShaAnnotation string) (RenovateConfig, error) {
	...
	packageRules = append(packageRules, PackageRule{
		...
		PRHeader:            fmt.Sprintf("Image created from '%s'", gitRepoAtShaAnnotation),
		RecreateWhen:        "always",
		RebaseWhen:          "behind-base-branch",
		Enabled:             true,
		FollowTag:           buildResult.BuiltImageTag,
		CommitBody:          fmt.Sprintf("Image created from '%s'\n\nSigned-off-by: %s", gitRepoAtShaAnnotation, target.GitAuthor),
...
func (u ComponentDependenciesUpdater) CreateRenovaterPipeline(ctx context.Context, namespace string, targets []updateTarget, debug bool, buildResult *BuildResult, gitRepoAtShaAnnotation string) error {
	log := logger.FromContext(ctx)
	...
	for _, target := range targets {
		...
		renovateConfig, err := GenerateRenovateConfigForNudge(target, buildResult, gitRepoAtShaAnnotation)
		if err != nil {
			return err
		}
